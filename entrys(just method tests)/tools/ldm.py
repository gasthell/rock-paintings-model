...
class LdmExtractor(nn.Module):
    ...
    def get_text_embeds(self, prompt, num_images_per_prompt=1, guidance_scale=1.0, img_size=1024):
        # --- FIX: Wrap the entire function in an autocast context ---
        # This ensures all operations within pipe.encode_prompt use the correct mixed precision
        with torch.autocast(device_type='cuda', dtype=torch.float16):
            ...
            if len(prompt_embeds_tuple) == 2:
                ...
                # This line is now safe because prompt_embeds.dtype will correctly be torch.float16
    ...
    def encode_to_latent(self, input_image):
        input_image = input_image.to(dtype=self.vae.dtype)
    ...
    def forward(self,
        ...
        # --- FIX 3: Add this line to handle dtype conversion automatically ---
        # This makes the class robust to different input dtypes.
        images = img.to(device=self.device, dtype=self.unet.dtype)
        
        images = F.interpolate(images, size=(img_size, img_size), mode='bilinear')
